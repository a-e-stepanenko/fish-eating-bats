...
tree_for_plot = ggplot(renamed_tree, aes(color=tree$label)) + 
  scale_color_manual(values=c("focus\nspecies"="#d1182a", "midground"="#d15500", "background"="#555555")) + 
...
gheatmap(tree_for_plot, table_bats, offset=18, width = 0.5, color = "black",, colnames_angle=90, hjust = 1) +
  ...
  theme(axis.text.x=element_blank(), legend.text = element_text(size=15), legend.title = element_text(size=15)) + 
  ggtree::vexpand(0, -1) + vexpand(-1)  + vexpand(-1)

## Gene losses TOGA2

gene_loss_full = read.csv("toga2_loss/lost_genes_16072025.txt", sep="\t", row.names = 1, header = T) %>% as.data.frame() # get gene states table generated by make_gene_dupl_status_table_with_genenames.py
gene_loss_full <- gene_loss_full[, !(colnames(gene_loss_full) %in% c("HLmyoViv2", "HLmyoViv3"))] # delete values for earlier versions of the Myotis vivesi assembly
